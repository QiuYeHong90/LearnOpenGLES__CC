...
struct ShaderTool {
    ...
    static func loadShaderFile(type:GLenum, fileName:String) -> GLuint {
        ...
        do {
            ...
            return ShaderTool.loadShaderString(type: type, shaderString: shaderString)
        } catch {
    ...
    static func loanProgram(verShaderFileName:String,fragShaderFileName:String) -> GLuint {
        ...
        let vertexShader = ShaderTool.loadShaderFile(type: GLenum(GL_VERTEX_SHADER), fileName: verShaderFileName)
        
        if vertexShader == 0 {return 0}
        
        let fragmentShader = ShaderTool.loadShaderFile(type: GLenum(GL_FRAGMENT_SHADER), fileName: fragShaderFileName)
