...
static void avStreamFPSTimeBase(AVStream *st, CGFloat defaultTimeBase, CGFloat *pFPS, CGFloat *pTimeBase)
{
    ...
    if (st->codecpar->frame_size != 1) {
        NSLog( @"WARNING: st.codec.ticks_per_frame=%d", st->codecpar->frame_size);
//        LoggerStream
        ...
        if (_formatCtx) {
            ...
            if (_videoStreams.count) {
                ...
                for (NSNumber *n in _videoStreams) {
                    ...
                    avcodec_string(buf, sizeof(buf), st->codecpar, 1);
...
{    
    ...
    AVCodecContext *codecCtx = _formatCtx->streams[videoStream]->codecpar;
...
{   
    AVCodecContext *codecCtx = _formatCtx->streams[audioStream]->codecpar;
...
{
    AVCodecContext *codecCtx = _formatCtx->streams[subtitleStream]->codecpar;
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    if (!_disableDeinterlacing &&
                        ...
                        avpicture_deinterlace((AVFrame*)_videoFrame,
                                              (AVFrame*)_videoFrame,
                                              _videoCodecCtx->pix_fmt,
                                              _videoCodecCtx->width,
                                              _videoCodecCtx->height);
